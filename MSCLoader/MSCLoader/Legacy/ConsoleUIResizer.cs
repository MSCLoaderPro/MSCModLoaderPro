...
namespace MSCLoader
{
    ...
    {
        RectTransform m_transform = null, m_logview = null, m_scrollbar = null;
        public GameObject logview, scrollbar;
        ...
        public void OnMouseEnter() => Cursor.SetCursor(cursor, new Vector2(24, 24), CursorMode.Auto);
        ...
        public void LoadConsoleSize()
        {
            ...
            if (File.Exists(Path.Combine(path, "console.data")))
            {
                try
                {
                    ...
                    if (float.Parse(values[0], CultureInfo.InvariantCulture) <= -60)
                    {
                        values[0] = "-60";
                        values[1] = "-60";
                        values[2] = "-60";
        ...
        public void SaveConsoleSize() => 
            File.WriteAllText(Path.Combine(ModLoader.GetModSettingsFolder(new ModConsole()), "console.data"), string.Format("{0},{1},{2},{3},{4}", m_transform.anchoredPosition.y, m_logview.anchoredPosition.y, m_scrollbar.anchoredPosition.y, m_logview.sizeDelta.y, m_scrollbar.sizeDelta.y));
    }
}